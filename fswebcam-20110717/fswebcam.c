...
int fswc_grab(fswebcam_config_t *config)
{
	...
	MSG("src.input is %s\n",src.input);
	MSG("src.tuner is %d\n",src.tuner);
	MSG("src.frequency is %d\n",src.frequency);
	MSG("src.delay is %d\n",src.delay);
	MSG("src.timeout is %d\n",src.timeout);
	MSG("src.use_read is %c\n",src.use_read);
	MSG("src.list is %d\n",src.list);
	MSG("src.palette is %d\n",src.palette);
	MSG("src.width is %d\n",src.width);
	MSG("src.height is %d\n",src.height);
	MSG("src.fps is %d\n",src.fps);
	//MSG("src.option.name is %s\n",src.option.name);
	...
	char *options = config->job[0]->options;
	MSG("options is %s\n",options);
...
int fswc_free_config(fswebcam_config_t *config)
{
	...
	if(config->dumpframe) free(config->dumpframe);
    if(config->title) free(config->title);
	if(config->subtitle) free(config->subtitle);
	//if(config->timestamp) free(config->timestamp);
	if(config->info) free(config->info);
...
int take_picture(char * name){
	uint32_t frame;
	uint32_t x, y;
	gdImage *image, *original;
	uint8_t modified;
	abitmap = calloc(task_src.width * task_src.height * 3, sizeof(avgbmp_t));
	if(!abitmap)
	{
		ERROR("Out of memory.");
		return(-1);
	...
	for(frame = 0; frame < task_config->frames; frame++)
	{
		if(src_grab(&task_src) == -1) break;

		
		/* Add frame to the average bitmap. */
		switch(task_src.palette)
		{
		case SRC_PAL_PNG:
			fswc_add_image_png(&task_src, abitmap);
			break;
		case SRC_PAL_JPEG:
		case SRC_PAL_MJPEG:
			fswc_add_image_jpeg(&task_src, abitmap);
			break;
		case SRC_PAL_S561:
			fswc_add_image_s561(abitmap, task_src.img, task_src.length, task_src.width, task_src.height, task_src.palette);
			break;
		case SRC_PAL_RGB32:
			fswc_add_image_rgb32(&task_src, abitmap);
			break;
		case SRC_PAL_BGR32:
			fswc_add_image_bgr32(&task_src, abitmap);
			break;
		case SRC_PAL_RGB24:
			fswc_add_image_rgb24(&task_src, abitmap);
			break;
		case SRC_PAL_BGR24:
			fswc_add_image_bgr24(&task_src, abitmap);
			break;
		case SRC_PAL_BAYER:
		case SRC_PAL_SGBRG8:
		case SRC_PAL_SGRBG8:
			fswc_add_image_bayer(abitmap, task_src.img, task_src.length, task_src.width, task_src.height, task_src.palette);
			break;
		case SRC_PAL_YUYV:
		case SRC_PAL_UYVY:
			fswc_add_image_yuyv(&task_src, abitmap);
			break;
		case SRC_PAL_YUV420P:
			fswc_add_image_yuv420p(&task_src, abitmap);
			break;
		case SRC_PAL_NV12MB:
			fswc_add_image_nv12mb(&task_src, abitmap);
			break;
		case SRC_PAL_RGB565:
			fswc_add_image_rgb565(&task_src, abitmap);
			break;
		case SRC_PAL_RGB555:
			fswc_add_image_rgb555(&task_src, abitmap);
			break;
		case SRC_PAL_Y16:
			fswc_add_image_y16(&task_src, abitmap);
			break;
		case SRC_PAL_GREY:
			fswc_add_image_grey(&task_src, abitmap);
			break;
		}
	}
	...
	/* Copy the average bitmap image to a gdImage. */
	original = gdImageCreateTrueColor(task_src.width, task_src.height);
	if(!original)
	{
		ERROR("Out of memory.");
		free(abitmap);
	...
	pbitmap = abitmap;
	for(y = 0; y < task_src.height; y++)
		for(x = 0; x < task_src.width; x++)
		{
			int px = x;
			int py = y;
			int colour;
			
			colour  = (*(pbitmap++) / task_config->frames) << 16;
			colour += (*(pbitmap++) / task_config->frames) << 8;
			colour += (*(pbitmap++) / task_config->frames);
	...
	task_config->bg_colour    = 0x40263A93;
	task_config->bl_colour    = 0x00FF0000;
	task_config->fg_colour    = 0x00FFFFFF;
	task_config->font         = strdup("sans");
	task_config->fontsize     = 10;
	task_config->shadow       = 1;
	task_config->title        = NULL;
	task_config->subtitle     = NULL;
	task_config->timestamp    = strdup("%Y-%m-%d %H:%M (%Z)");
	task_config->info         = NULL;
	task_config->underlay     = NULL;
	task_config->overlay      = NULL;
	task_config->filename     = NULL;
	task_config->format       = FORMAT_JPEG;
	task_config->compression  = -1;
	
	modified = 1;

	task_config->banner = NO_BANNER;

	fswc_output(task_config, name, image);
	modified = 0;
...
int open_camere(){
	...
	task_config = calloc(sizeof(fswebcam_config_t), 1);
	if(!task_config)
	{
		WARN("Out of memory.");
		return(-1);
	...
	memset(&task_src, 0, sizeof(task_src));
	//src.input 	 = config->input;
	task_src.tuner		= 0;
	task_src.frequency	= 0;
	task_src.delay		= 0;
	task_src.timeout	= 10; /* seconds */
	//src.use_read	 = '';
	task_src.list		= 0;
	task_src.palette	= -1;
	task_src.width		= Picture_Width;
	task_src.height 	= Picture_Height;
	task_src.fps		= 0;
	//src.option	 = config->option;
/*
	MSG("task_src.input is %s\n",task_src.input);
	MSG("task_src.tuner is %d\n",task_src.tuner);
	MSG("task_src.frequency is %d\n",task_src.frequency);
	MSG("task_src.delay is %d\n",task_src.delay);
	MSG("task_src.timeout is %d\n",task_src.timeout);
	MSG("task_src.use_read is %c\n",task_src.use_read);
	MSG("task_src.list is %d\n",task_src.list);
	MSG("task_src.palette is %d\n",task_src.palette);
	MSG("task_src.width is %d\n",task_src.width);
	MSG("task_src.height is %d\n",task_src.height);
	MSG("task_src.fps is %d\n",task_src.fps);
	//MSG("src.option.name is %s\n",src.option.name);
	//MSG("src.option.value is %s\n",src.option.value);
*/
	...
	task_config->width	= task_src.width;
	task_config->height = task_src.height;

	task_config->frames=1;
	task_config->skipframes=0;
	return(0);
...
int close_camera(){
	...
	/* Set the default values for this run. */
	if(task_config->font) free(task_config->font);
	if(task_config->title) free(task_config->title);
	if(task_config->subtitle) free(task_config->subtitle);
	if(task_config->timestamp) free(task_config->timestamp);
	if(task_config->info) free(task_config->info);
	if(task_config->underlay) free(task_config->underlay);
	if(task_config->overlay) free(task_config->overlay);
	if(task_config->filename) free(task_config->filename);
	src_close(&task_src);

	fswc_free_config(task_config);
	free(task_config);
	return(0);
